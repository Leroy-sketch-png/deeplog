...
def _write_json(target: Path, data: Any) -> None:
    _atomic_write(target, lambda p: p.write_text(json.dumps(data, indent=2), encoding="utf-8"))
...
def _train(train_events: List[Dict]) -> Dict:
    ...
    for i, e in enumerate(train_events):
        ...
        cs["hist"].append(op)
        elapsed = t - cs["start"]
        op_dur_sum[op]  += elapsed; op_dur_sq[op]  += elapsed ** 2; op_dur_cnt[op]  += 1
        op_idx_sum[op]  += cs["count"]; op_idx_sq[op] += cs["count"] ** 2; op_idx_cnt[op] += 1

        # Track B — caller session
        caller_ops[caller].add(op); caller_provs[caller].add(prov)
        caller_subs[caller].add(sub); caller_rgs[caller].add(rg)
        caller_types[caller].add(rtype); caller_ips[caller].add(ip)
        sess_id = int(t // 1800)
    ...
    return {
        "ngrams": ngrams, "max_ngram": max_ngram,
        "op_dur_mean": op_dur_mean, "op_dur_std": op_dur_std,
        "op_idx_mean": op_idx_mean, "op_idx_std": op_idx_std,
        "caller_ops": caller_ops, "caller_provs": caller_provs,
        "caller_subs": caller_subs, "caller_rgs": caller_rgs,
        "caller_types": caller_types, "caller_ips": caller_ips,
        "caller_mean_rate": caller_mean_rate, "caller_std_rate": caller_std_rate,
...
def _score(test_events: List[Dict], model: Dict) -> Tuple[Dict, Dict]:
    ...
    for e in test_events:
        ...
        if corr not in track_a_state:
            track_a_state[corr] = {"start": t, "end": t, "count": 0, "caller": caller,
                                    "hist": ["START"] * 4, "provs": set(), "rgs": set(),
                                    "subs": set(), "ops": []}
            track_a[corr] = {"struct": 0.0, "rarity": 0.0, "dur_dev": 0.0,
                              "len_dev": 0.0, "context": 0.0}
        cs = track_a_state[corr]
        sa = track_a[corr]
        cs["end"] = t; cs["count"] += 1
        cs["provs"].add(prov); cs["rgs"].add(rg); cs["subs"].add(sub)
        cs["ops"].append(op)
        ...
        dd  = min(abs(elapsed - op_dur_mean.get(op, elapsed)) / (op_dur_std.get(op, 1) + 1e-6) / 5, 1.0) if op in op_dur_mean else 1.0
        ld  = min(abs(seq_idx - op_idx_mean.get(op, seq_idx)) / (op_idx_std.get(op, 1) + 1e-6) / 5, 1.0) if op in op_idx_mean else 1.0
        ...
        if sess_key not in track_b:
            track_b[sess_key] = {"start": t, "end": t, "count": 0, "caller": caller,
                                  "new_op": 0.0, "new_prov": 0.0, "new_sub": 0.0,
                                  "new_rg": 0.0, "new_type": 0.0, "new_ip": 0.0,
                                  "ops": set()}
        sb = track_b[sess_key]
        sb["end"] = t; sb["count"] += 1; sb["ops"].add(op)
        if op   not in caller_ops.get(caller,   set()): sb["new_op"]   = 1.0
        if prov not in caller_provs.get(caller, set()): sb["new_prov"] = 1.0
        if sub  not in caller_subs.get(caller,  set()): sb["new_sub"]  = 1.0
        if rg   not in caller_rgs.get(caller,   set()): sb["new_rg"]   = 1.0
        if rtype not in caller_types.get(caller, set()): sb["new_type"] = 1.0
        if ip   not in caller_ips.get(caller,   set()): sb["new_ip"]   = 1.0
...
def _rank_and_write(track_a: Dict, track_a_state: Dict, track_b: Dict, output_dir: Path) -> None:
    ...
    for (caller, _), sb in track_b.items():
        ...
        if sb["act_dev"] > 0.8: parts.append("volume spike")
        if sb["hr_dev"]  > 0.8: parts.append("unusual hour")
...
def train_and_score(input_path: Path, output_dir: Path, dry_run: bool = False) -> None:
    ...
    logger.info(f"Split — Train: {len(train_events):,} | Val: skipped | Test: {len(test_events):,}")

    if not train_events:
        raise ValueError("Training set is empty. Check the input CSV timestamps.")
    if not test_events:
        raise ValueError("Test set is empty. Dataset may be too small for a 70/15/15 split.")
